...
const QuestList = ({ worldId, worldEmoji, quests }: QuestListProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Progress */}
      <div className={`p-5 border rounded ${progressGlow ? "shadow-lg" : ""}`}>
          ...
          {quests.map((quest) => {
            ...
            return (
              <div
                ...
                className={`p-3 border cursor-pointer ${done ? "opacity-60" : ""} ${
                  isAnimating ? "scale-105" : ""
                } ${isFlashing ? "bg-yellow-100" : ""}`}
              >
                <div className="flex items-center gap-3">
                  <input type="checkbox" checked={done} readOnly />
                  <span className={done ? "line-through" : ""}>{quest.title}</span>
                  <span
